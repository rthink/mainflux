...
var cmdChannels = []cobra.Command{
	...
	cobra.Command{
		Use:   "disconnnected",
		Short: "disconnnected <channel_id> <user_auth_token>",
		Long:  `List of Things disconnected from a Channel`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsage(cmd.Short)
				return
			...
			cl, err := sdk.ThingsByChannel(args[1], args[0], uint64(Offset), uint64(Limit), true)
			if err != nil {
				logError(err)
				return
			}
...
func NewChannelsCmd() *cobra.Command {
	cmd := cobra.Command{
		...
		Long:  `Channels management: create, get, update or delete Channel and get list of Things connected or disconnected from a Channel`,
		Run: func(cmd *cobra.Command, args []string) {
			logUsage("channels [create | get | update | delete | connections | disconnected]")
		},
	}
