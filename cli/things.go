...
var cmdThings = []cobra.Command{
	...
	cobra.Command{
		Use:   "disconnected",
		Short: "disconnected <thing_id> <user_auth_token>",
		Long:  `List of Channels disconnected from a Thing`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsage(cmd.Short)
				return
...
func NewThingsCmd() *cobra.Command {
	cmd := cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			logUsage("things [create | get | update | delete | connect | disconnect | connections | disconnected]")
		},
	}
